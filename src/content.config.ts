 ...
 * Normalize to string before regex validation.
 */
...
/** Loose date: full ISO date or year-month. */
...
const caseSchema = z
  ...
  .superRefine((data, ctx) => {
    ...
    data.amounts?.forEach((a, i) => checkRef(a.source_id, ['amounts', i, 'source_id']));
    ...
    const hasPrimary = data.sources.some((s) => s.primary);
    if (!hasPrimary) {
      ctx.addIssue({
        code: 'custom',
    ...
    const primaryKinds = data.sources.filter((s) => s.primary).map((s) => s.kind);
